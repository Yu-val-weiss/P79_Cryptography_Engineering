...
def test_compute(k: str, u: str, expected: str):
    assert X25519Base.compute_x25519_ladder(k, u) == expected
...
def test_iterated_compute(iters: int, expected: str):
    ...
    for _ in range(iters):
        old_k = k
        k = X25519Base.compute_x25519_ladder(k, u)
        u = old_k
